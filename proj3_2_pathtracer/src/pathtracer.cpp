...
Spectrum PathTracer::estimate_direct_lighting(const Ray& r, const Intersection& isect) {
  ...
  for (SceneLight *s: scene->lights) {
    ...
    for (int i = 0; i < num_samples; i++) {
      ...
      float distToLight;
      float pdf;
      Spectrum spect = s->sample_L(hit_p, &wi, &distToLight, &pdf);
      ...
      Ray r = Ray(EPS_D * wi + hit_p, wi);
      r.max_t = distToLight;

      if (!bvh->intersect(r)) {
        L_out += (isect.bsdf->f(w_out, w_in) * spect * w_in.z) / (pdf * (float) num_samples);
      }
    }
  }
...
Spectrum PathTracer::estimate_indirect_lighting(const Ray& r, const Intersection& isect) {
  ...
  double bigC = 10.0;
  double smallC = 0.05;
  double p = clamp(s.illum() * bigC + smallC, 0.0, 1.0);

  bool willContinue = coin_flip(p);

  if (willContinue) {
    Vector3D wi = o2w * w_in;

    Ray ray = Ray(EPS_D * wi + hit_p, wi);
    ray.depth = r.depth - 1;

    Spectrum spec = trace_ray(ray, isect.bsdf->is_delta());
    return (spec * s * abs_cos_theta(w_in)) / (pdf * p);
  } else {
    return Spectrum();
  }
...
Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
  ...
  for (samples_taken = 0; samples_taken < num_samples; samples_taken++) {
    Vector2D sample_point;
    if (num_samples == 1) {
      sample_point = Vector2D( (double) origin.x + 0.5, (double) origin.y + 0.5 );
    } else {
